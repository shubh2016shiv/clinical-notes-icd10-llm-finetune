...
uv run pyright <changed-files> # Static analysis for touched Python files
...
### 7.3.1 Pyright Guardrail

Pyright is installed through `uv` as a dev dependency and wired into pre-commit as a
local hook. It should be used for the same class of issues surfaced by Pylance/Cursor:
Protocol conformance, Literal narrowing, optional member access, union narrowing, and
attribute existence.
...
Run this before finalizing changes that touch protocols, Pydantic models, dependency
injection boundaries, factories, or result unions. The pre-commit hook runs Pyright
against staged Python files; do not bypass it. A full-package Pyright pass is useful
for backlog discovery, but may include unrelated existing debt. If a full run fails
outside your touched files, document the residual risk instead of hiding it.
...
- [ ] `uv run pyright <changed-python-files>` reports zero errors
